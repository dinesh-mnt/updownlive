...
function ArticleRow({ article }: { article: Article }) {
  ...
  const handleRoute = (e: React.MouseEvent) => {
    e.preventDefault();
    setArticle(`crypto_${article.id}`, article);
    ...
    <article className="border-b border-[#e8e8e8] dark:border-white/10 last:border-0">
      <div className="py-5 flex gap-5 items-start">
        <div className="flex-1 min-w-0">
          <h2
            className="text-xl md:text-2xl font-extrabold text-[#111111] dark:text-white leading-snug mb-2 cursor-pointer hover:text-brand-blue transition-colors"
            onClick={() => setExpanded(!expanded)}
          >
            {article.title}
          </h2>
          <p className="text-sm text-[#555] dark:text-gray-400 leading-relaxed mb-3 line-clamp-2">{article.subtitle}</p>
        ...
        <a href={article.url} onClick={handleRoute}
          ...
          <img src={article.imageUrl} alt={article.title}
            className="w-full h-[130px] md:h-[145px] object-cover rounded-lg border border-[#eaeaea] dark:border-white/10 hover:opacity-90 transition-opacity"
...
function ArticleGridCard({ article }: { article: Article }) {
  ...
  return (
    <a href={article.url} onClick={handleRoute}
      ...
      <div className="relative overflow-hidden h-44 bg-[#f5f5f5] dark:bg-zinc-800 flex-shrink-0">
        <img src={article.imageUrl} alt={article.title}
      ...
      <div className="flex flex-col flex-1 p-4 gap-2">
        <h3 className="font-extrabold text-[#111111] dark:text-white text-sm leading-snug line-clamp-3 group-hover:text-brand-blue transition-colors">
          {article.title}
        </h3>
        <p className="text-xs text-[#777] dark:text-gray-400 leading-relaxed line-clamp-2 flex-1">{article.subtitle}</p>
        <div className="flex items-center justify-between pt-2 border-t border-[#f0f0f0] dark:border-white/10 mt-1">
          ...
          <span className="text-xs text-[#aaa] dark:text-gray-500 flex items-center gap-1 flex-shrink-0">
  ...
  return (
    <div className="bg-white dark:bg-black min-h-screen font-sans">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-10">
        <h1 className="text-4xl font-extrabold text-[#111111] dark:text-white mb-6 tracking-tight">Cryptocurrency</h1>
        ...
        <div className="flex items-center gap-3 mb-6 pb-4 border-b border-[#e8e8e8] dark:border-white/10">
          ...
          <div className="flex items-center bg-[#f7f7f7] dark:bg-zinc-900 border border-[#e0e0e0] dark:border-white/10 rounded-xl p-1 gap-1 flex-shrink-0">
            <button onClick={() => setView("list")} title="List view"
              className={`p-2 rounded-lg transition-all ${view === "list" ? "bg-brand-blue text-white shadow-sm" : "text-[#888] dark:text-gray-400 hover:bg-[#e8e8e8] dark:hover:bg-zinc-800"}`}>
              <List size={15} />
            </button>
            <button onClick={() => setView("grid")} title="Grid view"
              className={`p-2 rounded-lg transition-all ${view === "grid" ? "bg-brand-blue text-white shadow-sm" : "text-[#888] dark:text-gray-400 hover:bg-[#e8e8e8] dark:hover:bg-zinc-800"}`}>
              <LayoutGrid size={15} />
            </button>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center py-20 text-center">
            ...
            <p className="font-bold text-[#111] dark:text-white">Failed to load articles</p>
            <p className="text-sm text-[#888] dark:text-gray-500 mt-1">{error}</p>
          </div>
        ) : paginated.length === 0 ? (
          <div className="flex flex-col items-center py-20 text-center">
