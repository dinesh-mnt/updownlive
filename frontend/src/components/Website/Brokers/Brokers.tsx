...
function parseBold(t: string) {
  return t.split("**").map((p, i) => i % 2 === 1 ? <strong key={i} className="font-bold text-[#111] dark:text-white">{p}</strong> : p);
}
    ...
    <article className="border-b border-[#e8e8e8] dark:border-white/10 last:border-0">
      <div className="py-5 flex gap-5 items-start">
        <div className="flex-1 min-w-0">
          <h2 className="text-xl md:text-2xl font-extrabold text-[#111] dark:text-white leading-snug mb-2 cursor-pointer hover:text-brand-blue transition-colors" onClick={()=>setOpen(!open)}>{a.title}</h2>
          <p className="text-sm text-[#555] dark:text-gray-400 leading-relaxed mb-3 line-clamp-2">{a.subtitle}</p>
          <div className="flex items-center justify-between gap-4 flex-wrap">
            <div className="flex items-center gap-2 flex-wrap">
              <span className="text-sm font-semibold text-brand-blue">{a.author}</span>
              <span className="text-[#ccc]">·</span>
              <span className="text-sm text-[#888] dark:text-gray-500 flex items-center gap-1"><Clock size={12}/>{timeAgo(a.publishedAt)}</span>
            ...
            <button onClick={()=>setOpen(!open)} className="flex items-center gap-1.5 text-sm font-bold text-[#333] dark:text-gray-300 hover:text-brand-blue transition-colors whitespace-nowrap">
        ...
        <a href={a.url} onClick={handleRoute} className="flex-shrink-0 w-[200px] md:w-[240px] hidden sm:block" tabIndex={-1}>
          <img src={a.imageUrl} alt={a.title} className="w-full h-[130px] md:h-[145px] object-cover rounded-lg border border-brand-border dark:border-white/10 hover:opacity-90 transition-opacity"
      ...
      {open && (
        <div className="pb-6 pr-0 sm:pr-[256px] animate-in fade-in slide-in-from-top-2 duration-200">
          <h3 className="text-xs font-black text-[#555] dark:text-gray-500 uppercase tracking-[0.15em] mb-4">Broker Overview</h3>
          <div className="space-y-4 text-[15px] text-[#333] dark:text-gray-300 leading-relaxed">{a.body.split("\n\n").filter(Boolean).map((p,i)=><p key={i}>{parseBold(p)}</p>)}</div>
...
function ArticleCard({ a }: { a: Article }) {
  ...
  return (
    <a href={a.url} onClick={handleRoute} className="group bg-white dark:bg-zinc-900 rounded-2xl border border-[#e8e8e8] dark:border-white/10 overflow-hidden hover:shadow-xl hover:shadow-brand-blue/8 hover:-translate-y-0.5 transition-all duration-200 flex flex-col">
      <div className="relative overflow-hidden h-44 bg-[#f5f5f5] dark:bg-zinc-800">
      ...
      <div className="flex flex-col flex-1 p-4 gap-2">
        ...
        <p className="text-xs text-[#777] dark:text-gray-400 leading-relaxed line-clamp-2 flex-1">{a.subtitle}</p>
        <div className="flex items-center justify-between pt-2 border-t border-[#f0f0f0] dark:border-white/10 mt-1">
          ...
          <span className="text-xs text-[#aaa] dark:text-gray-500 flex items-center gap-1"><Clock size={10}/>{timeAgo(a.publishedAt)}</span>
        </div>
      </div>
    </a>
  );
  ...
  return (
    <div className="bg-white dark:bg-black min-h-screen font-sans">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-10">
        <h1 className="text-4xl font-extrabold text-[#111] dark:text-white mb-6 tracking-tight">Brokers</h1>
        <div className="flex items-center gap-3 mb-6 pb-4 border-b border-[#e8e8e8] dark:border-white/10">
          ...
          <div className="flex items-center bg-[#f7f7f7] dark:bg-zinc-900 border border-[#e0e0e0] dark:border-white/10 rounded-xl p-1 gap-1 flex-shrink-0">
        ...
        {loading ? (
          <div className="space-y-1">{Array.from({length:5}).map((_,i)=>(
            <div key={i} className="py-5 border-b border-[#e8e8e8] dark:border-white/10 flex gap-5">
              <div className="flex-1 space-y-3"><div className="h-7 bg-[#f0f0f0] dark:bg-zinc-800 rounded-lg animate-pulse w-4/5"/><div className="h-4 bg-[#f5f5f5] dark:bg-zinc-900 rounded animate-pulse w-3/5"/><div className="h-3 bg-[#f5f5f5] dark:bg-zinc-900 rounded animate-pulse w-2/5"/></div>
              <div className="w-[200px] h-[130px] bg-[#f0f0f0] dark:bg-zinc-800 rounded-lg animate-pulse hidden sm:block"/>
            </div>))}</div>
        ) : paginated.length===0 ? (
          <div className="flex flex-col items-center py-20 text-center"><Newspaper size={40} className="text-[#ccc] mb-3"/><p className="font-bold text-[#111] dark:text-white">No articles found</p></div>
        ) : view==="list" ? (
