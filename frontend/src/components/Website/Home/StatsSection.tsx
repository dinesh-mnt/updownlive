...
export default function StatsSection() {
  return (
    <section className="py-20 bg-white border-b border-[#eaeaea] relative overflow-hidden">
      {/* subtle bg pattern */}
          ...
          {stats.map((stat, i) => (
            ...
            >
              ...
              <div className="flex items-start gap-4">
                <div className="p-3 rounded-xl bg-white/80 shadow-sm border border-white/60 backdrop-blur">
                  {stat.icon}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-2xl font-extrabold text-[#111111] tracking-tight leading-none mb-1">
                    {stat.value}
                  </p>
                  <p className="text-sm font-bold text-[#111111] mb-1">{stat.label}</p>
                  <p className="text-sm text-[#333333] leading-relaxed">{stat.description}</p>
                </div>
              </div>
            </div>
          ))}
